...
# --------------------------------------------------
        with col2:
            ...
            forecast = [("Mon", 45), ("Tue", 55), ("Wed", 70), ("Thu", 90), ("Fri", 120), ("Sat", 110), ("Sun", 80)]
            f_cols = st.columns(7)
            for i, (day, val) in enumerate(forecast):
                f_status = calculate_aqi(val)[1]
                f_bg = "#C8E6C9" if val <= 50 else "#FFE0B2" if val <= 100 else "#FFCDD2"

                with f_cols[i]:
                    # FIXED BOX: Strict height and overflow hidden to prevent "content coming out"
                    st.markdown(f"""
                        <div style="
                            background: {f_bg};
                            padding: 10px 2px;
                        ...
                        ">
                            ...
                            <div style="font-size: 22px; font-weight: 800; color: #000; margin: 10px 0;">{val}</div>
                            <div style="
                                height: 80px;
                                display: flex;
                                align-items: center;
